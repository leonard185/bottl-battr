// Sample data - replace with your actual bot data
const botData = [
    {
        "id": 101,
        "name": "wHz-93",
...
// Your enlisted bot army
let yourBotArmy = [];

// Function to render bot cards in the collection
function renderBotCollection() {
    const botCollection = document.getElementById('botCollection');
    botCollection.innerHTML = '';
    botData.forEach(bot => {
        ...
        botCard.innerHTML = `
            ...
            <button onclick="viewBotSpecs(${bot.id})">View Specs</button>
        `;
        botCollection.appendChild(botCard);
    });
}

// Function to render your enlisted bot army
function renderYourBotArmy() {
    const yourBotArmyDiv = document.getElementById('yourBotArmy');
    yourBotArmyDiv.innerHTML = '';
    yourBotArmy.forEach(bot => {
        const enlistedBot = document.createElement('div');
        enlistedBot.classList.add('enlisted-bot');
        enlistedBot.innerHTML = `
        <img src="${bot.avatar_url}" alt="Avatar">
        <div>Name: ${bot.name}</div>
        <div>Health: ${bot.health}</div>
        <div>Damage: ${bot.damage}</div>
        ...
        <div>Catchphrase: ${bot.catchphrase}</div>
            ...
            <button class="delete-button" onclick="deleteBot(${bot.id})">X</button>
        `;
        yourBotArmyDiv.appendChild(enlistedBot);
    });
}

// Function to enlist a bot into your army
function enlistBot(botId) {
    const bot = botData.find(bot => bot.id === botId);
    if (bot && !yourBotArmy.includes(bot)) {
        yourBotArmy.push(bot);
        renderYourBotArmy();
    }
}
...
function releaseBot(botId) {
    yourBotArmy = yourBotArmy.filter(bot => bot.id !== botId);
    renderYourBotArmy();
}

// Function to delete a bot forever
function deleteBot(botId) {
    botData = botData.filter(bot => bot.id !== botId);
    yourBotArmy = yourBotArmy.filter(bot => bot.id !== botId);
    renderBotCollection();
    renderYourBotArmy();
}

// Function to view bot specifications
function viewBotSpecs(botId) {
